...
> 1 | db.tasks.updateMany({status:"pending"},{$set{status:false}}}
    |                                             ^
  2 |
...
> 1 | db.tasks.find({Updated_at:{$gt new Date(new Date().getTime()-2*60*1000}})
    |                                ^
  2 |
...
11
>> Atlas atlas-2ig97m-shard-0 [primary] test> db.tasks.countDocuments({DeadLine:})
Uncaught:
SyntaxError: Unexpected token (1:34)

 > 1 | db.tasks.countDocuments({DeadLine:})
    |                                   ^
  2 |
